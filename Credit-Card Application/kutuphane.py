...
def dosya_oku(dosyaAdi):
    file_data = pd.read_csv('data/Credit_Card_Applications.csv')
    file_data=file_data.reset_index()
    #file_data=file_data.split()[0].replace('/', '')
    #file_data['CUST_ID'] = file_data['CUST_ID'].replace([":",','],"").astype(int)
...
def parametre_optimizasyonu(giris, cikis, CustomerID):
  ...
  ###############  
def basari_hesaplaCV(giris,cikis, CustomerID,cv=252):
    ...
    for train_index, test_index in folder.split(giris, cikis, CustomerID):
...
def basari_hesapla(giris,cikis, CustomerID):
    ...
    for train_index, test_index in logo.split(giris, cikis, CustomerID):
        ...
        toplamFSkor  += f1_score(y_test,pred_y)  
    #Ortalama Başarı = toplam başarı / parça sayısı
    return toplamBasari/logo.get_n_splits(giris, cikis, CustomerID), toplamFSkor/logo.get_n_splits(giris, cikis, CustomerID)
